...
! twc -- 2015-10-08 -- added variables to code
real:: efrac = 1.0 ! multiplier on latent heat flux, 1 by default
logical:: dosimplesfcflx = .false. ! option to use a much-simplified surface scheme
logical:: donudging_rad  = .false. !option to use nudging routine to apply simpilified radiative cooling
real:: qcool = 1.0   ! K/day cooling rate if donudging_rad=.true.
real:: ttpp  = 200.0 ! threshold 'tropopause' temperature above which sounding is cooled at rate qcool
real:: tau_nr = 40.0 ! time-scale of relaxation of tabs to ttpp if T<ttpp (days)

! twc -- 2017-01-11 - added variables to code
logical:: dosimplesflxconstk = .false. ! option to fix enthalpy fluxes in simplified surface scheme
real:: fluxq0const = 0.0 ! fixed surface latent heat flux (W/m^2) if dosimplesflxconstk=.true.
real:: fluxt0const = 0.0 ! fixed surface sensible heat flux (W/m^2) if dosimplesflxconstk=.true.
real:: simplesflxcd = 0.0015 ! drag coef if dosimplesfcflx=.true.
real:: simplesflxck = 0.0015 ! enthalpy exchange coef if dosimplesfcflx=.true.
